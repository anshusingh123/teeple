...
function node_setting($data, $operators, $tvshows) {    
    ...
    $node->field_tvshow[0]['nid'] = $tvshows[$data[6]];

    $start_datetime = $data[1]." ".sprintf('%02d', $data[2]).":".sprintf('%02d',$data[3]).":00";
    $end_datetime = $data[1]." ".sprintf('%02d', $data[4]).":".sprintf('%02d',$data[5]).":00";
...
$row = 1;
if (($handle = fopen("/var/www/tvchat/sites/default/files/Upload/cable.timetable.utf8.csv", "r")) !== FALSE) {
